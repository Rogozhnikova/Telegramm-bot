...
quest_steps = [
    ...
    },
    {
        ...
        "question": "Что написано внизу? (Напишите первые два слова)",
        "answer_type": "text",  # Тип ответа: варианты
        "correct_answer": "веленью божию",
        "hint": "Только два слова",  # Подсказка
    },
    {
    ...
    {
        "description": "🎉 Ура!Это финальное и самое интересное испытание! 🌟",
        "question": "Готов принять вызов?",
        "answer_type": "text",  # Тип ответа: варианты
        "correct_answer": "да",
    },
]
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if is_user_premium(user_id):
        ...
        await update.message.reply_text(
            "Чтобы повторить приключение вам необходимо 'Активировать доступ?' ниже.",
            reply_markup=create_main_keyboard()  # Отправляем клавиатуру
        )
...
async def send_step(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if step_index < len(quest_steps):
        ...
        await update.message.reply_text("🎉 Поздравляем! Вы успешно завершили квест и раскрыли все его тайны. Надеемся, вам понравилось это приключение! 🎉\n"
                                        ...
                                        "Оставьте честный отзыв о нашем заведении в одном из популярных сервисов («Яндекс», «2ГИС», Google). Сделайте скриншот отзыва, покажите его сотруднику заведения и  забирайте ваш заслуженный приз!")
